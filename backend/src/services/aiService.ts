...
const fallbackClassifier = (description: string): AIClassification => {
  ...
  if (numbers) {
    victimsCount = parseInt(numbers[0]);
...
export const classifyIncident = async (description: string): Promise<AIClassification> => {
...
"${description}"`
    ...
    const parsed: AIClassification = JSON.parse(jsonText);
    return parsed;
  } catch (error) {
